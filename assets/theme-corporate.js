...
(function(){
    ...
    QtySelector.prototype = Object.assign({}, QtySelector.prototype, {
      init: function() {
        ...
        $(".remove-cart-item").click(function () {
            var id = $(this).closest(".cart__item.cart_custom_item").attr("data-key");            
            $.ajax({
            type: "POST",
            url: "/cart/change.js",
            dataType: "json",
            ...
            success: function () {
            // $(this).find('.js-qty__wrapper .js-qty__num').val("0");
  ...
  theme.QuickAdd = (function() {
    var selectors = {
      ...
      quickAddBtn_custom: '.custom_quick_add',
...
//    $(document).on('click', '.custom_recom .custom_quick_add', function(){
//         console.log('clccc');
//         });
...
//         var quickAddBtns_c = this.container.querySelectorAll(selectors.quickAddBtn_custom);

//         if (quickAddBtns_c){
// 		//$('body').on('click','.custom_quick_add', this.addToCart.bind(this));
//           quickAddBtns_c.forEach(btn => {
//             console.log(btn);
//             $(document).on('click','.custom_quick_add', this.addToCart.bind(this));
// //             $(document).on('click','.custom_quick_add', function(){
// //             console.log('hii');
// //             });
//           });
//         }
...
// 		 }, 1000);
   ...
   $(document).on('click', '.custom_recom .custom_quick_add', function(evt){
     var btn = evt.currentTarget;
      var visibleBtn = btn.querySelector('.btn');
        visibleBtn.classList.add('btn--loading');
...
// 		 }, 1000);
   ...
   $(document).on('change', '.quantity_dropdown select', function(evt){
     var thnmain = $(this);
      var value = $(this).find('option:selected').attr('value');
      var val_int = parseInt(value);
     
     var nval = $(this).find('option:selected').text();
     if (nval.indexOf('+') > -1){
       $(this).parents('.grid-item.grid-product').find('.styled-select select').hide();
       $(this).parents('.grid-item.grid-product').find('.text_input_filed').show();
       $(this).parents('.grid-item.grid-product').find('.quantity_if_plus').val(value);
     }

     $(this).parents('.grid-item.grid-product').find('.quick-add-btn.js-quick-add-btn_custom').attr('data-qty',value);
     var get_price = parseInt($(this).parents('.grid-item__meta').find('.grid-product__price--current').attr('data-price'));
     var pn = get_price*value;
   //  $(this).parents('.grid-item__meta').find('.grid-product__price--current span').text(theme.Currency.formatMoney(pn, theme.settings.moneyFormat));

     $(this).parents('.grid-item.grid-product').find('.bss-b2b-qty-table-row').each(function(){
       var ths = $(this);
       var price_val = ths.find('td:last-child').text();
       var min_data = parseInt($(this).attr('data-min'));
       var max_data = parseInt($(this).attr('data-max'));
        if (val_int >= min_data && val_int <= max_data){
          // console.log(price_val);
          thnmain.parents('.grid-item__meta').find('.grid-product__price--current span').text(price_val);
           thnmain.parents('.grid-item__meta').find('.onwords_text').text('Per Unit');
            return false;
        }
   ...
   $(document).on('change', '.quantity_if_plus', function(evt){
            var thnmain = $(this);
      var value = $(this).val();
             var val_int = parseInt(value);
     $(this).parents('.grid-item.grid-product').find('.quick-add-btn.js-quick-add-btn_custom').attr('data-qty',value);
 ...
 $(document).on('change', '.quantity_dropdown_product select', function(evt){
     var thnmain = $(this);
      var value = $(this).find('option:selected').attr('value');
      var val_int = parseInt(value);
     ...
     if (nval.indexOf('+') > -1){
       ...
       $('.text_input_filed').show();
       $('.quantity_if_plus_custom').val(value);
     }

    $('.product__quantity .js-qty__num').val(value);
     //$('.quick-add-btn.js-quick-add-btn_custom').attr('data-qty',value);
 
     var pn = get_price*value;
   //  $(this).parents('.grid-item__meta').find('.grid-product__price--current span').text(theme.Currency.formatMoney(pn, theme.settings.moneyFormat));

     $('table.orichi-table.orichi-table-pricetype-1 tbody tr').each(function(){
       var ths = $(this);
       var price_val = ths.find('.orichi-money').text();
       var min_data = parseInt($(this).find('.orichi-buy').text());
       var max_data = parseInt($(this).next().find('.orichi-buy').text());
       //alert('min_data ' + min_data);
       //alert('max_data ' + max_data);
        if (val_int >= min_data && val_int <= max_data){
         console.log("pricemm " + price_val);
          $('.product__price_main').text("Discount Price " + price_val +" /each");
          $('.product__price_main').attr('data-save-price', price_val);
           $('.oncwords_cs').text('( Per Unit )');
            return false;
        }
       else if(val_int > max_data){
          var price_val_last = $('.orichi-table').find('tr:last-child').find('td:last-child').find('.orichi-money').text();

          $('.product__price_main').text("Discount Price " + price_val_last +" /each");

         $('.product__price_main').attr('data-save-price', price_val_last);
           $('.oncwords_cs').text('( Per Unit )');
       }
     });
       var get_price = $(this).parents('.product-single__meta').find('.product__price_main').attr('data-save-price');
   var newprice = get_price.replace('₹ ','').replace(',', '').replace('.', '');
   
      console.log(newprice);

   var compareprice = $(this).parents('.product-single__meta').find('.custom_com_price').text();
   var newcomprice = compareprice.replace('₹ ','');
      console.log(newcomprice);

   var saveprice = parseInt(newcomprice) - parseInt(newprice);

   $(this).parents('.product-single__meta').find('.product__price-savings').text('save ₹ ' + saveprice);
   console.log('save ' + saveprice);
     });


   
      $(document).on('change', '.quantity_if_plus_custom', function(evt){
       var thnmain = $(this);
      var value = $(this).val();
      var val_int = parseInt(value);
     var nval = $(this).find('option:selected').text();
     if (nval.indexOf('+') > -1){
      ...
      if ($(this).val() < min)
        {
        $(this).val(min);
         var price_val_last = $('.orichi-table').find('tr:first-child').find('td:last-child').text();
         $('.product__price_main').text(price_val_last);
         $('.oncwords_cs').text('( Per Unit )');
        }  
     $('.product__quantity .js-qty__num').val(value);
     var get_price = parseInt($(this).parents('.grid-item__meta').find('.grid-product__price--current').attr('data-price'));
   ...
   //  $(this).parents('.grid-item__meta').find('.grid-product__price--current span').text(theme.Currency.formatMoney(pn, theme.settings.moneyFormat));
     ...
     $('table.orichi-table.orichi-table-pricetype-1 tbody tr').each(function(){
       ...
       var min_data = parseInt($(this).find('.orichi-buy').text());
       var max_data = parseInt($(this).next().find('.orichi-buy').text());
       console.log('val ' + val_int);
       console.log('val ' + min_data);
        if (val_int >= min_data && val_int <= max_data){
         console.log("price" + price_val);
          $('.product__price_main').text(price_val);
           $('.oncwords_cs').text('( Per Unit )');
            return false;
        }
       else if(val_int > max_data){
         console.log('mmmm val' + max_data);
        var price_val_last = $('.bss-b2b-qty-table-wrapper').find('tr:last-child').find('td:last-child').text();
         $('.product__price_main').text(price_val_last);
         $('.oncwords_cs').text('( Per Unit )');
       }
     });

   var get_price = $(this).parents('.product-single__meta').find('.product__price_main').text();
   var newprice = get_price.replace('₹ ','').replace(',', '').replace('.', '');
  ...
  $(document).on('click', '.order_sample_button', function(evt){
   var btn = evt.currentTarget;
    var visibleBtn = btn.querySelector('.btn');
      visibleBtn.classList.add('btn--loading');
   var id = $(this).attr('data-id');
...
document.addEventListener('cart:update', function(evt) {
      if (evt && evt.detail && evt.detail.section) {
        new theme.QuickAdd(evt.detail.section);
        new theme.QuickShop(evt.detail.section);
      }
...
// $('.cart__checkout-wrapper .cart__checkout').click(function(event){
//   event.preventDefault();
  
//       var cartTotal = $('.cart__item--subtotal .orichi-quantity-cart').text();
//   var totalPrice = cartTotal.substring(1, cartTotal.length);
//   alert(totalPrice);

//   if (totalPrice < 2,499) {
//     $('.cart__checkout-wrapper .cart__checkout').addClass('disabled');
//   }
...
$('.cart__checkout-wrapper .cart__checkout').click(function(event) {
  ...
  var cartTotal = $('.cart__item--subtotal .orichi-quantity-cart').text();
  var totalPrice = parseFloat(cartTotal.replace(',', '').substring(1)); // Remove commas and currency symbol, and convert to a number
  alert(totalPrice);

  if (totalPrice < 2499) { // Changed 2,499 to 2499
    $('.cart__checkout-wrapper .cart__checkout').addClass('disabled');
    $('.cart__item-row.cart__checkout-wrapper.payment-buttons').prepend('<div class="alert_message"><small>The cart price should be greater than or equal to 2499 to place an order.</small></div>');
    return false;
  }
  else {
    $('.cart__checkout-wrapper .cart__checkout').removeClass('disabled');
    $('.cart__checkout-wrapper.payment-buttons .alert_message').css('display', 'none');
  }
});
